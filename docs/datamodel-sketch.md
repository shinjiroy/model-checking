...
/** 整数値の式。atom値(Term)とは型で分離しており、両者を混同するASTは組み立てられない */
type IntExpr =
  | { kind: "lit"; value: number }                              // 整数リテラル
  | { kind: "card"; relation: string }                          // 関係のタプル数(濃度)
  | { kind: "count"; sort: string; varId: number; body: Formula } // 集合内包の濃度 #{x: sort | body(x)}
  | { kind: "add"; left: IntExpr; right: IntExpr };             // 加算
...
`and` / `or` / `not` / `implies` / `iff` / `lit(n)` / `card(relationName)` / `count(sort, x => body)` /
`add(a, b, ...rest)` / `lt` / `le` / `gt`(`lt`の引数入れ替えの糖衣) / `ge`(`le`の引数入れ替えの糖衣)。

- **変数の内部表現**: de Bruijn指数ではなく、束縛(`forall`/`exists`/`count`の呼び出し)のたびに一意なidを
  振る方式を採った。コールバックへ渡す`Term`にそのidが埋め込まれており、コールバックが組み立てた式(`body`)を
  そのままASTへ格納するだけでよく、指数の付け替え計算が要らない。結果のASTはid・ソート名だけを持つプレーン
  オブジェクトになり、「JSONシリアライズ可能」という要件を満たす
- **タグ付きユニオン**: 各ノードは`kind`で判別できる。ユニオンにケースを追加し、評価器・(将来の)Z3翻訳器に
  そのケースのハンドラを足すだけで拡張できる設計であり、算術・集合濃度(`lit`/`card`/`count`/`add`/`lt`/`le`)は
  この方式でケースを追加して収載した。既存のノード形は変更していない
- **atom値とint値の型分離**: `Term`(atomが動く値)と`IntExpr`(整数値)は別の型であり、`eq`/`rel`の引数に
  `IntExpr`を渡す、`lt`/`le`の引数に`Term`を渡す、といった取り違えは型検査の時点で弾かれる
- **現在の語彙の範囲**: 量化・等値・関係・論理結合子に加え、有限領域上の線形整数算術(整数リテラル・関係の濃度・
  集合内包の濃度・加算・`<`・`<=`)を収載している。乗算・非有界量化・実数といった、有限ソート上のZ3のInt+ite
  総和に収まらない語彙は意図的に含めていない。Z3の表現力とこの語彙の範囲をおおよそ一致させることで、
  語彙を広げてもZ3翻訳できないノードが混ざらないようにしている
