# 実設計での検証(成功基準1): RealWorld/Conduit + 自チーム設計

GOAL.mdの成功基準1「自チームの実設計を1つモデル化し、レビューで見逃された問題を1件以上発見できる」に向けた検証。
ツールの存在意義を確認する最重要タスクとして、二本立てで進める。

- **トラックA**: RealWorld(Conduit)を concept 単位でモデル化した、公開・再現可能なショーケース(本ドキュメントの主対象)
- **トラックB**: 自チームの実設計の検証(成功基準1の本命。チーム内部設計の情報が要るため枠組みのみ提示)
...
Daniel Jackson は Alloy の作者であり、本ツールが GOAL.md で参照する形式手法系譜の中心にあたる。
...
## トラックB: 自チームの実設計(成功基準1の本命)

トラックA単独では「レビューで見逃されたバグの発見」の説得力に欠ける(RealWorldは公開題材で既知の論点が多い)。
成功基準1を満たすには、**非同期・リトライ・状態遷移を含む自チームの実設計を1つ選んでモデル化し、
...
4. 反例が出たら、レビューで見逃されていたかを確認する(見逃しバグ1件でツールの存在意義が立つ)
